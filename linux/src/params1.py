...
def params(bit_rate,station_id,label, ensID, ensLabel, service):
    ...
    command=(f"sudo xterm -hold -e 'sudo ./repos/crc-dabmux/src/CRC-DabMux -i {ensID} -L {ensLabel} -A ./pipes/f1.fifo -b {bit_rate} -i {station_id} -p 3 -S -L {label} -i {service} -C -i1 -O fifo://pipes/s1.fifo'")
    os.system(command)

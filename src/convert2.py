...
def mp3tofifo(mp3_name, sample_rate, bit_rate):
    ...
    command = (f"xterm -hold -e 'sudo ffmpeg -stream_loop -1 -re -i music/{mp3_name}.mp3 -ar {sample_rate} -f wav - |sudo ./repos/toolame-02l/toolame -s {sample_rate_short} -D 4 -b {bit_rate} /dev/stdin ./pipes/f2.fifo'")
    os.system(command)
